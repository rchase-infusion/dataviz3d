...
{
    class SimpleAllEdges : EdgeGenerator
    {
        private int count1;
        ...
        public SimpleAllEdges(int count1, int centerId1, int count2, int centerId2)
        {
            this.count1 = count1;
        ...
        private List<EdgeRawData> EdgeRawDatas(int count, int centerId)
        {
            ...
            {
                var id = centerId + i + 1;
                result.Add(new EdgeRawData(id, "Edge " + id, centerId, id, GetRandomColorColor(), (float) (.004 * random.NextDouble())));
            }
            return result;
        }
    }
